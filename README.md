...
| `beq <rs1>, <rs2>, <k>`     | Остановить процессор  Перейти на адрес pc + k, <br/>если содержимое rs1 меньше или равно содержимому rs2. Для сравнения знаковых чисел |
...
| `ble <rs1>, <rs2>, <k>`     |       3       | `k -> r31`,<br/>`rs1 - rs2 -> NZ`,<br/>`if N then pc + r31 -> pc`   |
...
| `latch_rr`            | Защелкнуть операнды команды из `Instruction Decoder`                                     |
...
| `sel_mpc_inc_two_if_n` | Управляющий сигнал для выбора 2 в качестве инкремента для `MPC` если `N` == 1            |
...
- `latch_rr` - защелкнуть значение в `OR`
- `latch_mpc` - защелкнуть значение в `MPC`
