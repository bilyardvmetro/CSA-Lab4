...
<instruction> ::=
               ...
               | "blе" <register> "," <register> "," <label> [ <comment> ]
               | "halt" [ <comment> ]
               | <label> [ <comment> ]
...
| `blе <rs1>, <rs2>, <k>`  | B-type | `if rs1 < rs2 then pc <- pc + k`  | Перейти на адрес pc + k, <br/>если содержимое rs1 меньше или равно содержимому rs2       |
...
| `blе <rs1>, <rs2>, <k>`     |       3       | `k -> r31`,<br/>`rs1 - rs2 -> NZ`,<br/>`if NZ=10 then pc + r31 -> pc` |
...
- `Instruction decoder` - декодер бинарной инструкции. Содержит Look Up Table маппинга инструкций на микрокоманды.
  Определяет индекс начальной для инструкции микрокоманды в Look Up Table, отправляет в `Signal Dispatcher` индексы
  регистров, а `imm/offset` значения в `DataPath`
- `IR (Index Register)` - регистр для хранения индекса Look Up Table
- `MPC (MicroProgram Counter)` - счетчик микропрограмм
- `MP memory` - память микропрограмм
- `Signal Dispatcher` - осуществляет выполнение микропрограммы, анализ флагов результата и отправляет управляющие
  сигналы в `DataPath`

Процесс выполнения программы:

1. На вход `CU` поступает бинарная инструкция из памяти инструкций
2. По совокупности типа команды, опкода и расширение опкода из Look Up Table достается индекс микрокоманды для
   инструкции. Индексы регистров уходят в `Signal Dispatcher`, `imm/offset` значения в `DataPath`
3. Индекс из Look Up Table уходит в `IR` операнды в `OR`
...
- `sel_mpc_inc` - селектор инкремента `mpc` (mpc + 1, mpc + 2). Нужен для Branch-Type команд, которые зависят от флагов
  `NZ`
- `sel_mpc` - выбрать следующее значение для `mpc` (mpc inc, Look Up Table Index from `IR`, 0)
