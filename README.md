...
- По умолчанию индексы `mmioIn` и `mmioOut` равны `0` и `1` соответственно. Ячейки можно конфигурировать через константы
  `mmioIn` и `mmioOut` в файле [memory.go](machine/memory.go)
...
2. По совокупности типа команды, опкода и расширение опкода из `LookUpTable` достается индекс микропрограммы для
